    ...
    <AppLayout>
      ...
      <div className="absolute inset-x-0 top-0 z-10 px-5 pt-14 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`h-1.5 w-8 rounded-full transition-all ${step >= 0 ? "bg-[#6C5CFF]" : ""}`} style={step >= 0 ? undefined : { backgroundColor: "var(--bg-input)" }} />
          <div className={`h-1.5 w-8 rounded-full transition-all ${step >= 1 ? "bg-[#6C5CFF]" : ""}`} style={step >= 1 ? undefined : { backgroundColor: "var(--bg-input)" }} />
        </div>
        <button
        ...
        {step === 1 && (
          ...
          >
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-black italic" style={{ color: "var(--text-primary)" }}>你的口味偏好</h1>
              <p className="mt-1.5 text-[12px] font-medium" style={{ color: "var(--text-muted)" }}>挑 3~5 个，AI 会优先推荐相关地点</p>
            </div>
                ...
                {INTERESTS.map((it) => {
                  ...
                  return (
                    <button
                      ...
                      style={on ? undefined : { backgroundColor: "var(--bg-input)" }}
                    >
                      <Icon size={20} className={on ? "text-[#A98BFF]" : ""} style={on ? undefined : { color: "var(--text-muted)" }} />
                      <span className={`text-[14px] font-bold ${on ? "text-white" : ""}`} style={on ? undefined : { color: "var(--text-secondary)" }}>{it.label}</span>
                    </button>
                  );
                })}
              </div>
              {interests.length > 0 && (
                <p className="mt-4 text-center text-[11px]" style={{ color: "var(--text-faint)" }}>已选 {interests.length} 个</p>
              )}
            </div>
          </motion.div>
        )}
