...
export function EventDetailScreen({ onBack }: { onBack: () => void }) {
  return (
    <AppLayout>
      <div className="absolute inset-0 bg-[#070718]">
      ...
      <main className="absolute inset-0 flex flex-col items-center justify-center px-8 text-center pt-10">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative mb-8">
          <div className="relative z-10 flex h-40 w-40 items-center justify-center rounded-full border border-white/20 bg-[#1E1E4C] shadow-[0_0_60px_rgba(108,92,255,0.3)]">
        ...
        <h1 className="text-[28px] font-black italic tracking-tighter text-white">触发：城市盲盒</h1>
        <p className="mt-2 text-[15px] text-white/60">你在长宁区某条无名小巷内发现了一份“惊喜”</p>

        <div className="mt-10 w-full space-y-3">
          <EventAction icon={<Sparkles size={20} className="text-[#FFD166]" />} title="立即开启" subtitle="消耗 10 能量" color="bg-gradient-to-r from-[#6C5CFF] to-[#8F5CFF]" />
          <EventAction icon={<Gift size={20} className="text-white/60" />} title="稍后开启" subtitle="放入背包" color="bg-white/5 border border-white/10" />
        </div>
      </main>
      
      <div className="absolute bottom-[30px] left-6 right-6">
        <Glass className="p-4 text-left border border-[#FFD166]/20">
          <div className="flex items-center gap-2 mb-1 text-[#FFD166] text-[13px] font-bold">概率公示</div>
          <p className="text-[12px] text-white/50">传说级：1.5% / 史诗：8.5% / 普通：90%</p>
        </Glass>
      </div>
    </AppLayout>
  );
...
function EventAction({ icon, title, subtitle, color, glow = "" }: { icon: React.ReactNode; title: string; subtitle: string; color: string; glow?: string }) {
  return (
    <motion.button 
